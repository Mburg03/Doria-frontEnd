...
const Sidebar = ({ mobileOpen = false, onClose = () => {} }) => {
    ...
    if (user?.role === 'admin') {
        navItems.push({ name: 'Administración de usuarios', icon: LayoutDashboard, path: '/admin/users' });
    }
        ...
        <div className="h-full w-64 bg-white border-r border-gray-200 flex flex-col">
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            ...
                            onClick={onClose}
                        >
                            <item.icon size={20} className={clsx('mr-3', isActive ? 'text-gray-500' : 'text-gray-400')} />
                            {item.name}
                        </Link>
                    );
                })}
            ...
            <div className="p-4 border-t border-gray-100">
                <div className="space-y-2">
                    ...
                    >
                        {loggingOut ? <Loader2 size={18} className="mr-3 animate-spin" /> : <LogOut size={18} className="mr-3" />}
